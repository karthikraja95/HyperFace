...
def create_hyperface_network():
    ...
    model = Model(inputs=input, outputs=[face_output, landmarks_output, visibility_output])
    ...
    loss_weights = {'face_detection_out': 1, 'landmarks_output': 5.0, 'visibility_output': 0.5}
...
def hyperface_callbacks():
    ...
    #callback_early_stopping = EarlyStopping(monitor='val_landmarks_output_loss', patience=10, verbose=1)
...
def generator(x, y, batch_size=32):
    ...
    while True:
        ...
        while batch_start < data_size:
            ...
            yield batch_x, [batch_f, batch_l, batch_v]
...
x_train, x_test, y_train_landmarks, y_test_landmarks, y_train_visibility, y_test_visibility, y_train_face,y_test_face = train_test_split(images, landmarks, visibility,face,test_size = 0.20, shuffle = True)
...
if exists:
    ...
    callbacks = hyperface_callbacks()
    labels = [y_train_face, y_train_landmarks, y_train_visibility]
    labels_test = [y_test_face, y_test_landmarks, y_test_visibility]
